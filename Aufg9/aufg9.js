...
(function (Canvas) {
    ...
    function init(_e) {
        ...
        let cbaum = document.getElementsByTagName("canvas")[0];
    ...
    function way() {
        ...
        let cp1 = { x: 100, y: 800 };
        let cp2 = { x: 1000, y: 600 };
        let end = { x: 1600, y: 1000 };
        c2d.bezierCurveTo(cp1.x, cp1.y, cp2.x, cp2.y, end.x, end.y);
    ...
    function snowman() {
        c2d.beginPath();
        let x = Math.floor(Math.random() * 100) + 1500;
        let y = Math.floor(Math.random() * 100) + 1000;
    ...
    function snowflakes() {
        for (let i = 0; i < 500; i++) {
            let x = Math.random() * c2d.canvas.width;
            let y = Math.random() * c2d.canvas.height;
            singleflake(x, y);
        }
    }
    function singleflake(_x, _y) {
